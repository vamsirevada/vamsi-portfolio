    ...
    >
      <div className="grid grid-cols-1 gap-[clamp(40px,6vw,80px)] md:grid-cols-2">
          ...
          <div className="relative pl-7">
            ...
            {timelineItems.map((item) => (
              <div
                key={item.year}
                data-reveal="true"
                className="relative pb-9 opacity-0 transition-[opacity,transform] duration-700 ease-out"
        ...
        <div id="process">
          ...
          <div className="flex flex-col gap-px overflow-hidden rounded-[20px] border border-white/8 bg-white/6">
            {processSteps.map((step) => (
              <div
                key={step.n}
                data-reveal="true"
                className="flex translate-y-4 items-start gap-[18px] bg-card px-6 py-[22px] opacity-0 transition-[opacity,transform,background] duration-600 ease-out hover:bg-[#161616]"
              >
